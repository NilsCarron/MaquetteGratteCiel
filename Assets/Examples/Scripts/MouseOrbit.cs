...
public class MouseOrbit : MouseNavBase {
    ...
    public override void Update() {
        ...
        if (mouseDiff.x != 0)
        {
            cam.RotateAround(pivot, cam.up,  accel*Time.deltaTime * mouseDiff.x);
        }

        if (mouseDiff.y != 0)
        {
            cam.RotateAround(pivot, -cam.right, rotSpeed *Time.deltaTime * mouseDiff.y);
        }
        ...
        {
            cam.RotateAround(pivot, cam.forward, rotSpeed *Time.deltaTime * mouseDiff.w);
        }
        ...
        if (mouseDiff.z != 0)
        {
            ...
            cam.Translate(-cam.forward * d *Time.deltaTime, Space.World);
        }
    }
}
